...
const rewards = [
  ...
  {
    title: "Early Adopter",
    desc: "Activated benefits in first week",
    icon: iconTrendingUp,
    earned: true,
    points: 150,
  },
  {
    ...
    desc: "Used wellbeing allowance 3 months in a row",
  ...
  {
    title: "Team Player",
    desc: "Referred a colleague to a benefit",
    icon: iconCheckCircle,
    earned: false,
    points: 100,
  },
  {
    title: "Full Coverage",
    desc: "Activate all available benefits",
    icon: iconTrendingUp,
    earned: false,
    points: 500,
  },
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-[1000px]">
      <motion.div variants={fadeUp} className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Earn points by engaging with your benefits</p>
      </motion.div>

      {/* Stats */}
      <motion.div variants={fadeUp} className="grid grid-cols-3 gap-4 mb-10">
        <div className="happl-card p-6">
          <p className="text-xs font-semibold tracking-wider text-muted-foreground mb-2">TOTAL POINTS</p>
          <AnimatedCounter value={totalPoints} className="text-4xl font-bold text-accent" />
          <p className="text-sm text-muted-foreground mt-1">Available to redeem</p>
        </div>
        <div className="happl-card p-6">
          <p className="text-xs font-semibold tracking-wider text-muted-foreground mb-2">BADGES EARNED</p>
          <AnimatedCounter value={earnedCount} className="text-4xl font-bold text-foreground" />
          <p className="text-sm text-muted-foreground mt-1">of {rewards.length} total</p>
        </div>
        <div className="happl-card p-6">
          <p className="text-xs font-semibold tracking-wider text-muted-foreground mb-2">CURRENT STREAK</p>
          <div className="flex items-baseline gap-1">
            <AnimatedCounter value={12} className="text-4xl font-bold text-foreground" />
            <span className="text-lg text-muted-foreground">weeks</span>
          </div>
          <p className="text-sm text-muted-foreground mt-1">Benefits engagement</p>
        </div>
      </motion.div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {r.earned && (
                        <span className="happl-badge bg-accent/15 text-accent text-[10px]">Earned</span>
                      )}
                    </div>
                    <p className="text-sm text-muted-foreground">{r.desc}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-foreground">{r.points}</p>
