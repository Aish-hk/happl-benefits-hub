...
export default function EnrollmentFlow() {
  ...
  const [dependents, setDependents] = useState([
    { name: "Emily Smith", relation: "Spouse" },
  ]);
  ...
  const addDependent = () => {
    setDependents([...dependents, { name: "", relation: "Child" }]);
  };
        ...
        >
          {step === 0 && (
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Choose the coverage level that works best for you
              </p>
              <div className="grid grid-cols-3 gap-4">
                {plans.map((plan, i) => (
                  <motion.div
                    ...
                    className={`relative happl-card cursor-pointer transition-all ${
                      selectedPlan === i
                  ...
                  >
                    {plan.recommended && (
                      <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 happl-badge-accent text-[10px]">
                        Recommended
                      </span>
                    )}
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Add family members to cover under your plan
              </p>
              ...
              <div className="space-y-3">
                ...
                {dependents.map((dep, i) => (
                  ...
                  >
                        ...
                        <div>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              newDeps[i].name = e.target.value;
                              setDependents(newDeps);
                            }}
                            placeholder="Full name"
                            className="text-sm font-medium text-foreground bg-transparent border-b border-border pb-1 focus:outline-none focus:border-accent"
                          />
                        </div>
                      </div>
                      <button
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const newDeps = [...dependents];
                        newDeps[i].relation = e.target.value;
                        setDependents(newDeps);
                      }}
                      className="text-sm text-foreground bg-secondary rounded-md px-3 py-1.5 border-none focus:outline-none focus:ring-2 focus:ring-accent"
                    >
                ...
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-border rounded-xl text-sm text-muted-foreground hover:border-accent hover:text-accent transition-all"
                >
                  <Plus size={16} />
          ...
          {step === 3 && (
            ...
            >
              ...
              <p className="text-muted-foreground mb-8 max-w-md mx-auto">
                ...
                <strong>{plans[selectedPlan].name}</strong> plan has been
                submitted. Coverage begins May 1, 2026.
              </p>
              <div className="flex items-center justify-center gap-3">
                <button
                  onClick={() => navigate("/")}
                  className="px-6 py-2.5 rounded-lg bg-accent text-accent-foreground text-sm font-semibold hover:brightness-95 transition-all"
                >
                  Back to Home
                </button>
                <button
                  onClick={() => navigate("/marketplace")}
                  className="px-6 py-2.5 rounded-lg border border-border text-sm font-medium text-foreground hover:bg-secondary transition-all"
                >
      ...
      {step < 3 && (
        ...
        >
          ...
          <motion.button
            ...
            className="flex items-center gap-2 bg-accent text-accent-foreground px-6 py-2.5 rounded-lg text-sm font-semibold hover:brightness-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            {step === 2 ? "Confirm Enrollment" : "Continue"}
            <ArrowRight size={16} />
          </motion.button>
        </motion.div>
      )}
