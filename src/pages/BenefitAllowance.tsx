      ...
      <motion.div variants={fadeUp} className="rounded-2xl bg-gradient-to-r from-[hsl(var(--accent))] to-[hsl(157,70%,40%)] p-8 mb-8">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-5">
            <img src={iconFlexAllowance} alt="Flex Allowance" className="w-28 h-28 object-contain" />
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl text-primary">Flex Allowance</h1>
                <span className="happl-badge bg-primary/15 text-primary text-xs">Active</span>
              </div>
              <p className="text-primary/70 mt-1 font-light">Spend on wellness, fitness, learning & lifestyle</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-primary/60 font-light">Available this month</p>
            <AnimatedCounter value={remaining} prefix="€" className="text-3xl font-semibold text-primary" />
            <p className="text-xs text-primary/50 font-light">of €150/month</p>
          </div>
        </div>
        <div className="mt-6">
          <div className="flex justify-between text-xs text-primary/60 mb-1.5 font-light">
      ...
      <div className="grid grid-cols-5 gap-6">
        <div className="col-span-3">
          <motion.div variants={fadeUp}>
            <h2 className="text-xl text-foreground mb-4">Spend by Category</h2>
            <div className="grid grid-cols-2 gap-3">
              {spendingCategories.map((cat, i) => (
                <motion.div key={cat.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 + i * 0.08 }} whileHover={{ scale: 1.02 }} className="happl-card flex items-center gap-3 cursor-pointer">
                  <div className={`w-10 h-10 rounded-xl ${cat.color} flex items-center justify-center text-lg`}>{cat.icon}</div>
                  <div className="flex-1">
                    <p className="text-sm font-medium text-foreground">{cat.label}</p>
                    <p className="text-xs text-muted-foreground font-light">€{cat.spent} last month</p>
                  </div>
          ...
          <motion.div variants={fadeUp} className="mt-8">
            <h2 className="text-xl text-foreground mb-4">Recent Transactions</h2>
                ...
                <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.06 }} className="flex items-center justify-between py-3.5 first:pt-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-light">{tx.date} · {tx.category}</p>
                    </div>
                  </div>
                  <span className={`text-sm font-medium ${tx.amount > 0 ? "text-accent" : "text-foreground"}`}>
        ...
        <motion.div variants={fadeUp} className="col-span-2 space-y-4">
            ...
            <ol className="space-y-3">
              {["Browse eligible categories", "Make a purchase or book a service", "Upload your receipt in the app", "Get reimbursed within 5 business days"].map((step, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-accent/15 flex items-center justify-center text-xs font-medium text-accent shrink-0">{i + 1}</span>
                  <p className="text-sm text-muted-foreground font-light">{step}</p>
                </li>
              ))}
            </ol>
          </div>
          <div className="happl-card-static">
            <h3 className="font-medium text-foreground mb-3">Eligible Spending</h3>
          ...
          <button onClick={() => navigate("/enroll/flex-allowance")} className="w-full flex items-center justify-center gap-2 bg-accent text-accent-foreground py-3 rounded-lg text-sm font-medium hover:brightness-95 transition-all">
            Submit a Receipt
          </button>
        </motion.div>
      </div>
