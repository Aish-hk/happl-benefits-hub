...
const spendingCategories = [
  { label: "Fitness & Gym", icon: Dumbbell, spent: 65, color: "bg-accent" },
  { label: "Food & Nutrition", icon: Utensils, spent: 35, color: "bg-happl-warning" },
  { label: "Learning", icon: BookOpen, spent: 20, color: "bg-happl-info" },
  { label: "Equipment", icon: Monitor, spent: 30, color: "bg-[#A855F7]" },
];
...
export default function BenefitAllowance() {
  ...
  const spent = 150;
  const remaining = totalBudget - spent + 150; // carried over
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={fadeUp}
        className="rounded-2xl bg-gradient-to-r from-[#04E898] to-[#02b574] p-8 mb-8"
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-primary/70 mt-1">
                Spend on wellness, fitness, learning & lifestyle
              </p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-primary/60">Available this month</p>
            <p className="text-3xl font-semibold text-primary">€{remaining}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {spendingCategories.map((cat, i) => (
                <motion.div
                  ...
                  className="happl-card flex items-center gap-3 cursor-pointer"
                >
                  <div className={`w-10 h-10 rounded-xl ${cat.color}/15 flex items-center justify-center`}>
        ...
        <motion.div variants={fadeUp} className="col-span-2 space-y-4">
          <div className="happl-card-static">
            ...
            <ol className="space-y-3">
              {[
                "Browse eligible categories",
                "Make a purchase or book a service",
                "Upload your receipt in the app",
                "Get reimbursed within 5 business days",
              ].map((step, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-accent/15 flex items-center justify-center text-xs font-semibold text-accent shrink-0">
                    {i + 1}
                  </span>
          ...
          <div className="happl-card-static">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={item}
                  className="happl-badge-muted text-xs"
                >
                  {item}
                </span>
              ))}
          ...
          <button
            onClick={() => navigate("/enroll/flex-allowance")}
            className="w-full flex items-center justify-center gap-2 bg-accent text-accent-foreground py-3 rounded-lg text-sm font-semibold hover:brightness-95 transition-all"
          >
            Submit a Receipt
          </button>
        </motion.div>
